    ...
    <div className="flex flex-wrap items-center gap-2 text-[10px]">
      ...
      {sources.map((s, i) => {
        ...
        return (
          <div key={i} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/[0.04] border border-white/[0.08]">
            ...
            <Database className="w-3 h-3 text-purple-300/50" />
            <span className="text-purple-100 font-semibold tracking-wide">{s.label}</span>
            {s.detail && <span className="text-purple-300/40 font-mono">{s.detail}</span>}
          </div>
        );
      })}

      {/* Última atualização */}
      <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/[0.04] border border-white/[0.08] text-purple-200/60">
        <Clock className="w-3 h-3" />
        <span>Atualizado <span className="text-white/80 font-semibold">{formatRelative(lastFetched)}</span></span>
        {nextRefresh && <span className="text-purple-300/40">· próximo {formatUntil(nextRefresh)}</span>}
      </div>
      ...
      {onRefresh && (
        <button
          ...
          className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-purple-500/10 hover:bg-purple-500/20 border border-purple-400/20 text-purple-200 transition-colors disabled:opacity-50"
        >
