...
## Now
ROLLOUT DO PORTAO (opcao 2 escolhida pelo Andre 02/09 ~20:30). Commit A 0e19d71 (38 chamadores mandam secret) e commit B 2d14739 (portao no .gs, INERTE) prontos, NAO publicados. Ordem obrigatoria: (1) git push -> Vercel no ar com A; (2) Andre cria ~/.j26_admin_secret (chmod 600, mesmo ADMIN_SECRET da Vercel); (3) verificar /api/slots, /api/galeria GET, POST /api/videos-checkout contra v67; (4) bash scripts/deploy-gs.sh -> v68; (5) verificar: sem secret -> {"error":"nao autorizado"}, ping ok, Vercel 200, scripts locais ok. Provado 20:52 que a v67 ignora ?secret= (passo 1 seguro). ACHADO DE SEGURANCA P1: repo e PUBLICO e o deploymentId esta commitado em 5 arquivos; ?action=bookings devolve 179 clientes (nome, e-mail, WhatsApp, NOME DA BAILARINA) sem credencial nenhuma. Handoff de 02/09 respondido (P1-P4). Campanha 55/55 completa, plist removido, backup do video5678 verificado byte-a-byte. P1 (conector Drive) segue bloqueado apesar do André ter relinkado — reportar e tentar de novo numa sessão nova. INCIDENTE RESOLVIDO NA CAUSA: o PIX que falha no Mercado Pago é incidente do ASAAS (status.asaas.com, 31/08→02/09: troca do certificado de assinatura do QR; MP/Inter/PagSeguro ainda recusando em 02/09 08:40). Não é nosso código. Ação: avisar clientes (cartão no mesmo link, ou Pix por outro banco). O bug do timeout de 8 s (b60d553) é real, mas SEPARADO e não é a causa das reclamações de hoje.
...
## Done
- Retry de entrega na fila (pedidos5678.py, fora do git) — RESULT: rodada no-op limpa com a Queila ja entregue; ramo exercitado de verdade com o pedido de TESTE (entregue_para apagado -> checar reentregou ao Andre).
- PRIMEIRA VENDA REAL — Queila Galdino Ferraz (AG-MRNLDJ0W), 1 video (020), R$120, pay_bim34zg7uw48xmw8, pago 02/09 13:29. Fila produziu o 4K as 15:31; ENTREGA FALHOU as 15:33 (bookings 404 2x) e NAO foi reprocessada (.produzido.json sai da varredura). Reexecutado a mao as ~18:00: entregue para queilatr@hotmail.com, token 2b084b706e5e7436. 7,5 h de atraso que ninguem teria visto sem ler o launchd.log.
...
## Open items
- RESOLVIDO 02/09 ~18:05: pedidos5678.py checar() agora reentrega todo .produzido.json sem entregue_para (ver Done). Era: entrega que falha NUNCA e reprocessada (pedidos5678.py: .produzido.json com entregue_para=None fica invisivel). Aconteceu HOJE com a primeira venda real. Fix: checar() tambem tenta entregar todo .produzido.json sem entregue_para. O 404 do redirect do Google esteve severo a tarde (6 falhas seguidas 13:01-17:18) — 2-3 tentativas nao bastam.
- NOTED (not done): src/App.tsx:159 SHEETS_URL aponta para um deployment MORTO (AKfycbwU..., devolve HTML de erro do Google) - o formulario de leads submitToSheets falha em silencio (no-cors). Nao e vazamento. Corrigir depois.
